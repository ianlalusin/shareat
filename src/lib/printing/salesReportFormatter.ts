...
export function formatSalesReportText(data: SalesReportData, width: 58 | 80 = 80): string {
  ...
  // --- Remittance Summary ---
  lines.push(center("REMITTANCE SUMMARY"));
  lines.push(hr());
  const cashLabel = width === 58 ? "Cash Remitted" : "Total Cash Remitted";
  const onlineLabel = width === 58 ? "Online Remitted" : "Total Online Remitted";
  lines.push(justify(cashLabel, fmtAmt(data.cashRemitted)));
  lines.push(justify(onlineLabel, fmtAmt(data.onlineRemitted)));
  lines.push(hr());
